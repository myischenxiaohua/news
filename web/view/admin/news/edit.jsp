...
<div id="main">
    ...
    <div id="opt_area">
        ...
        <form  action="${pageContext.request.contextPath}/new.html?action=save" enctype="multipart/form-data" method="post">
                ...
                <select name="ntid">
                    <c:forEach items="${topic}" var="tp">
                        <c:choose>
                            ...
                            <c:when test="${news.ntid==tp.tid}">
                                <option value='${tp.tid}' selected="selected">${tp.tname}</option>
                            </c:when>
                            <c:otherwise>
            ...
            <p>
                <label> 内容 </label>
                <textarea name="ncontent" id="ncontent" style="width:600px;height:500px;">${news.ncontent}</textarea>
            </p>
            <p>
                <label> 上传图片 </label>
                <input name="file" id="file" type="file" class="opt_input" />
            </p>
            <img src="${pageContext.request.contextPath}/upload/${news.npicpath}">
